...
def pass_decompose(m: QModule, ctx: PassContext) -> None:
    ...
    Decompose high-level ops:
      ...
      - q.iqft(reg, reverse) → ...
      - q.diffusion(reg) → H^n · X^n · MCX(all) · X^n · H^n (MCX left as q.mcx_all marker for later lowering)
    """
        ...
        for bb in fn.blocks:
            ...
            while i < len(bb.ops):
                ...
                if op.op == "q.iqft":
                    ...
                    i += len(repl); continue
                if op.op == "q.diffusion":
                    r = op.args[0]
                    repl: List[QOp] = []
                    # H layer
                    for i_ in range(n_qubits):
                        repl.append(QOp("q.h", args=(r, str(i_))))
                    # X layer
                    for i_ in range(n_qubits):
                        repl.append(QOp("q.x", args=(r, str(i_))))
                    # Multi-controlled Z about |1...1> implemented as MCX(all) marker (to be lowered later)
                    repl.append(QOp("q.mcx_all", args=(r,)))
                    # X layer
                    for i_ in range(n_qubits):
                        repl.append(QOp("q.x", args=(r, str(i_))))
                    # H layer
                    for i_ in range(n_qubits):
                        repl.append(QOp("q.h", args=(r, str(i_))))
                    _replace_ops(bb, i, repl); ctx.changed = True
                    ctx.log.append(f"decompose: DIFFUSION → {len(repl)} ops incl. mcx_all")
                    i += len(repl); continue
                i += 1
