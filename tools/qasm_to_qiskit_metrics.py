...
def analyze(qc: QuantumCircuit):
    ...
    def barrier_touch(qubits):
        for q in qubits: t_block[q]=True
    for inst, qargs, cargs in qc.data:
        ...
        if name in ('t','tdg','z','rz','s','sdg'):
            if name in ('t','tdg'):
                q = qargs[0].index
                if t_block[q]:
                    tstage_by_qubit[q]+=1; t_block[q]=False
                stats[name]+=1
            continue
        if name in ('h','x'):
            q = qargs[0].index
            if name in stats: stats[name]+=1
            depth_by_qubit[q] = depth_by_qubit[q] + 1
            barrier_touch([q]); continue
        if name=='cx':
            a=qargs[0].index; b=qargs[1].index
            stats['cx']+=1
            ...
            barrier_touch([a,b]); continue
        if name in ('ccx','toffoli'):
            a=qargs[0].index; b=qargs[1].index; c=qargs[2].index
            stats['ccx']+=1
    ...
    return dict(depth=depth, twoq_depth=twoq_depth, twoq_count=twoq_count, twoq_equiv=twoq_equiv, tcount=tcount, tdepth=tdepth, **stats)
