...
class Interpreter:
    ...
    def __init__(self, program: Program):
        if program.n_qubits is None: raise ValueError("Program must ALLOCATE before run")
        self.p=program; self.state=StateVector(program.n_qubits)
        self.env={}; self.cbits={}; self.counts=None
    def _idx(self, expr: str) -> int:
        val = int(round(eval(expr, {"__builtins__": None}, {})))
        if not (0 <= val < self.p.n_qubits): raise IndexError(f"qubit index {val} out of range")
        return val
    def _run_text(self, text: str):
        sub=Program(text).parse(); it=Interpreter(sub)
        it.state=self.state; it.env=self.env; it.cbits=self.cbits
        it.run(); self.state=it.state; self.env=it.env; self.cbits=it.cbits; self.counts=it.counts
    def _cond(self, expr: str) -> bool:
        s=expr
        for name,val in self.cbits.items():
            s = s.replace(name, "1" if val else "0")
        try: return bool(eval(s, {"__builtins__": None}, {}))
    ...
    def run(self):
        for ins in self.p.instructions:
            ...
            if op in ("H","X","Z"):
                ...
                continue
            if op=="RZ_EXPR":
                q=self._idx(args[1]); theta=float(eval(args[2], {"__builtins__":None, "pi":3.1415926535}))
            ...
            if op=="DIFFUSION":
                ...
                if n>=2: self.state.apply_two(n-2,n-1,CNOT_4)
                for q in range(n): self.state.apply_single(q,X)
                for q in range(n): self.state.apply_single(q,H)
                continue
            if op=="MEASURE_ONE_EXPR":
                q=self._idx(args[1]); 
                c = self.state.sample_all(1); bit=list(c.keys())[0][q]; self.cbits[args[2]]=int(bit); continue
            if op=="MEASURE_ALL":
